 ...
 * 미진열 사유를 뽑아낸다.
 *
 * ⚠️ 서버의 `latestHideInfo`는 **displayStatus가 HIDDEN일 때만** 채워진다
 * (ProductProcessingHistoryService.getLatestHideInfo). 브랜드 요청으로 내려간
 * HIDE_REQUEST 상태에서는 항상 null이라, 그 경우엔 처리 이력에서 직접 찾아야 한다.
 */
function resolveHideReason(
  ...
  if (
    ...
    // 재검토 대기는 "브랜드의 정보 수정"이 트리거라 새로 지적된 사유가 없다(§11-11)
    return null
  }

  if (detail.latestHideInfo) {
    const info = detail.latestHideInfo
    return {
      reasonType: info.hideReasonType,
      detail: info.hideDetail,
      at: info.hiddenAt,
      actor: info.processorName,
    }
  }

  const fromHistory = (detail.processingHistory ?? []).find(
    item =>
      item.historyType === "HIDE_REQUESTED" || item.historyType === "HIDDEN"
  )
  if (!fromHistory) {
    return null
  }
  ...
  return {
    reasonType: fromHistory.hideReason?.reasonType ?? "BRAND_REQUEST",
    detail: fromHistory.hideReason?.detail ?? null,
    at: fromHistory.createdAt,
    actor: fromHistory.processorName,
  }
}
        ...
        <Card title="미진열 사유" tone={isBrandRequest ? "neutral" : "danger"}>
          <div className="mb-3">
            ...
            <div className="text-[12px] font-medium text-sz-n-900">
              {getHideReasonLabel(hideReason.reasonType)}
            </div>
          </div>
          {hideReason.detail && (
            <div className="mb-3">
          ...
          <div className="text-[11px] text-sz-n-500">
            {[formatDateTimeShort(hideReason.at), hideReason.actor]
              .filter(Boolean)
              .join(" · ")}
          </div>
        </Card>
