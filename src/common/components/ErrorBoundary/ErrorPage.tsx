...
import { ROLE, getMenuTypeByRole } from "@/common/types/role"
    ...
    <div className="mx-auto w-full max-w-3xl">
      <div className="rounded-lg bg-white p-6 shadow-sm">
        ...
        <p className="mb-4 text-sm text-sz-n-600">
          예기치 않은 오류가 발생했습니다. 새로고침하거나 잠시 후 다시
          시도해주세요.
        </p>
        {error && (
...
export default function ErrorPage({ error: propError }: Props) {
  ...
  // 에러 화면은 스토어가 비어 있는 시점(초기 로드 중 크래시)에도 그려져야 한다 — 셀러 메뉴로 폴백
  const menuType = getMenuTypeByRole(role ?? ROLE.SELLER)
  const menus = [
    menuType === "SELLER" ? SELLER_MENU : CREATOR_MENU,
    COMMON_MENU,
  ]
